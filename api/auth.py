...
async def login_with_github():
    ...
    # Added prompt=select_account so the user can pick from multiple GitHub accounts
    url = f"https://github.com/login/oauth/authorize?client_id={settings.GITHUB_CLIENT_ID}&scope=repo,admin:repo_hook&prompt=select_account"
...
async def github_callback(code: str):
    ...
    try:
        ...
        # Redirect back to the Vercel frontend with a success flag
        response = RedirectResponse(url=f"{settings.FRONTEND_URL}/?logged_in=1")
        ...
        return RedirectResponse(f"{settings.FRONTEND_URL}/?error=oauth_exception")
