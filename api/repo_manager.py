...
@router.post("/api/unregister-repo")
async def unregister_repository(request: RepoRegistrationRequest, fastapi_req: Request):
    """
    Unregisters a repository by deleting the webhook on GitHub and purging the DB record.
    ...
    use_token = fastapi_req.cookies.get("github_token") or settings.GITHUB_TOKEN
    if not use_token:
        raise HTTPException(status_code=401, detail="Please Sign in with GitHub first to disconnect a repository.")

    webhook_target_url = f"{settings.BACKEND_PUBLIC_URL.rstrip('/')}/api/webhook"
    
    success = github_api.delete_webhook(
        repo_name=repo_name,
        webhook_url=webhook_target_url,
        token=use_token
    )
    
    # Regardless of GitHub success (webhook might already be dead), clean our DB
    remove_repo_token(repo_name)
    
    return {
        "status": "success", 
        "repo": repo_name, 
        "message": f"Successfully disconnected {repo_name}!"
    }
