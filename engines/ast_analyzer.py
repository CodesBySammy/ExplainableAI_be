...
class ASTEngine:
    def scan(self, raw_code: str, filename: str, tree: ast.AST = None) -> dict:
        ...
        # 1. Catch Syntax Errors early before running Radon or AST Visitors
        try:
            if tree is None:
                tree = ast.parse(raw_code)
        except SyntaxError as e:
            logger.warning(f"Syntax error found in {filename}: {e}")
            report_data["suggestions"].append({
                "type": "error",
                "title": "Syntax Error Detected",
                "description": f"The Python file `{filename}` contains invalid syntax preventing structural analysis.",
                "line": getattr(e, 'lineno', 'Unknown'),
                "original": str(getattr(e, 'text', 'N/A')).strip() if getattr(e, 'text', None) else "N/A",
                "fix": "# Please fix the syntax error to enable XAI Code Review."
            })
            return report_data
        ...
        # 4. Custom Enterprise AST Visitor
        try:
            scanner = ASTAutoFixer(raw_code)
            scanner.visit(tree)
            report_data["suggestions"] = scanner.suggestions
        except Exception as e:
            logger.error(f"AST Visitor failed for {filename}: {e}")
