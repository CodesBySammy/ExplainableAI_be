...
class GitHubClient:
    ...
    def create_webhook(self, repo_name: str, webhook_url: str, secret: str, token: str = None) -> bool:
        """Create or update a webhook on a target repository"""
        url = f"{self.base_url}/repos/{repo_name}/hooks"
    ...
    def delete_webhook(self, repo_name: str, webhook_url: str, token: str = None) -> bool:
        """Deletes the specific webhook from the target repository."""
        url = f"{self.base_url}/repos/{repo_name}/hooks"
        try:
            res = self.session.get(url, headers=self._get_headers(token), timeout=self.TIMEOUT)
            if res.status_code != 200:
            ...
            if not hook_id_to_delete:
                logger.warning(f"Webhook {webhook_url} not found on {repo_name} to delete.")
                return True 
                
            delete_url = f"{self.base_url}/repos/{repo_name}/hooks/{hook_id_to_delete}"
            del_res = self.session.delete(delete_url, headers=self._get_headers(token), timeout=self.TIMEOUT)
            
            if del_res.status_code == 204:
                logger.info(f"🗑️ Successfully deleted webhook from {repo_name}")
                return True
            else:
                logger.error(f"Failed to delete webhook {hook_id_to_delete}: HTTP {del_res.status_code}")
                return False
